...
2. **Chain of Responsibility** para cálculo de daño: `CadenaDanio` compone 7 manejadores (base, tipo, STAB, crítico, posición, clima, objetos equipados — `ManejadorObjetosEquipados` con mapa `['life_orb' => 1.30]`, reemplazó a `ManejadorOrbeVida`). Fácil de extender con nuevos modificadores.
...
12. **Deuda de arquitectura del módulo Battle (Arquitecto APROBADO, 2026-08-30)** — ver detalle en `src/Battle/context.md`:
    ...
    - `Combate.php` es god-component (653 líneas); `Combatiente`/`AgregadoBatalla` son god-classes (606 y ~255 líneas tras extraer `CalculadorDañoClima`/`SelectorAccionIA`).
    - Cobertura Infection de `src/Battle`: Covered Code MSI ~80% (mutantes escapados de `SelectorAccionIA` son equivalentes/neutros); pendiente cerrar el 20% restante.
...
- `docs/analysis/pokemon-showdown/` — Análisis de referencia de Pokemon Showdown (8 archivos: arquitectura, fórmulas, mecánicas de batalla, datos estáticos, patrones de diseño, recomendaciones de migración e índice cruzado de navegación rápido PS↔proyecto). Fuente externa para decisiones de diseño del motor de batalla; no es un port directo.
