...
return new class () extends Migration {
    ...
    {
        Schema::table('reclutables', function (Blueprint $table) {
            $table->unsignedBigInteger('user_id')->nullable()->after('id');
        });

        $legacyUserId = LegacyUserMigrator::ensureLegacyUserId();
        if ($legacyUserId !== null) {
            DB::table('reclutables')->whereNull('user_id')->update(['user_id' => $legacyUserId]);
        }
        ...
        Schema::table('reclutables', function (Blueprint $table) {
            ...
            $table->index('user_id');
            // La captura pasa a ser por jugador: unique(user_id, pokemon_id).
